...
@login_required
def backend_uav_model_edit(request):
	...
	if not staff:
		...
		if request.method == 'POST':###有数据提交时执行
			...
			uavmodel_test = UAV_Model.objects.filter(uav_model=uav_model)
			if uavmodel_test and uavmodel_test[0].id!=uavmodel.id:
		...
		return render_to_response("backend_uav_model_edit.html",{
			"self":request.user,
			"uavmodel":uavmodel,
			},
