...
@login_required
def backend_uav_model_manage(request):
	...
	if not staff:
		...
		if 'batch_delete' in request.POST:##批量删除
			id_selected = request.POST.getlist('uav_model_manage_selected','')
			for i in id_selected:
				delete_list.append(i)
			delete_user = UAV_Model.objects.filter(id__in = delete_list)
			for one in delete_user:
				one.delete()
				
		uavmodel = UAV_Model.objects.all()
		uavmodelcount = len(uavmodel)
